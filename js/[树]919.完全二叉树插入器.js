...
var CBTInserter = function (root) {
  this.rootNode = root;
  const [count, level] = traverseTree(root);
  this.count = count;
  this.currentLevel = level - 1;
  if (this.count === Math.pow(2, this.currentLevel + 1) - 1) {
    this.currentLevel++;
  }
};
...
function traverseTree(node) {
  ...
  const [lCount, lLevel] = traverseTree(node.left);
  const [rCount, rLevel] = traverseTree(node.right);
  return [1 + lCount + rCount, Math.max(1 + lLevel, 1 + rLevel)];
}
...
CBTInserter.prototype.insert = function (v) {
  ...
  let index = this.count - Math.pow(2, this.currentLevel) + 1;
  let levelCount = Math.pow(2, this.currentLevel);
